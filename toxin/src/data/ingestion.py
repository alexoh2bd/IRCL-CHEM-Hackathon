...
def build_prompt(question, response):
    with open("masterprompt.yaml") as f:
        raw_text = f.read()
    variables = {
        "Question": question,
        "Response": response,
    }
    text = Template(variables).safe_substitute(variables)
    return text
...
if __name__ == '__main__':
    ...
    # print(data_dir)
    df =pd.read_csv(data_dir / "chemlit_qa_train.csv")
    ...
    for i in range(1):
        prompt = build_prompt(df['Question'][i], df['Answer'][i])
